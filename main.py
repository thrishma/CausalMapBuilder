...
def proceed_intent_request(intent, session):
    question = session['attributes']['question']
    if('proceed' in question[len(question)-1]):
        print("sending the pointer to no entity")
        return handle_session_end_request(intent, session, proceed = True)
    else:
        card_title = 'Do not provide single word answers'
        should_end_session = False
        speech_output = "Do not provide single word answers, please answer in a proper sentence. For example, you can answer by saying: I think it is caused by..."
        reprompt_text = speech_output
        question.append(reprompt_text)
        session_attributes.update({
                'question':question,
...
def model_creator_intent(intent, session):
    ...
    if(len(current_question_word)>1):
        if((current_question_word in identified_entities.keys()) and len(identified_entities.keys()) > 0):
            identified_entities_list_extension = list(set(identified_entities[current_question_word]))
        else:
            identified_entities_list_extension = []
    else:
        identified_entities_list_extension = []

    print("identified_entities_list_extension:",identified_entities_list_extension)
    ...
    if(answer_from_user == ''):
        speech_output = "Sorry,I was unable to capture the entites in your answer. Please repeat your answer, clearly or provide another answer"
        reprompt_text = speech_output
        question.append(reprompt_text)
        session_attributes.update({
                'question':question,
    ...
    if(len(question)==2):
        response = answer_from_user
    #-------------------google API--------------------------#

    entities_count = session['attributes']['entities_count']
    identified_entities_list =[]
    current_entities = []
#---------------------------------start of capturing core concept entities-----------------------------------------------------------------#
    if(len(question)==2):
        if not response:
            speech_output = "Sorry,I was unable to capture the entites in your answer. Please frame your answer, by saying - I would like the core concept to be, followed by the core concept of your choice"
            reprompt_text = speech_output
            question.append(reprompt_text)
            session_attributes.update({
                    'question':question,
            ...
            if(response not in syn_dict):
                identified_entities_list.append(response)
                first_layer_entities.append(response)
            else:
                identified_entities_list.append(response)
                
#---------------------------------end of capturing core concept entities-----------------------------------------------------------------#
        ...
        entities_list = []
        if not response:
            adj = syntax_text(answer_from_user)
            print("adj:",adj)
            if not adj:
                speech_output = "Sorry,I was unable to capture the entites in your answer. Please repeat your answer, clearly or try providing me another answer"
                reprompt_text = speech_output
                question.append(reprompt_text)
                session_attributes.update({
                        'question':question,
            ...
            for entity in response:
                print("response in after core concept ",entity)                
...
#----------End of Check if any entities captured in this round already exists in the values of already captured derivational values---------------#
    ...
    for value in identified_entities_list:
        identified_entities_list_extension.append(value)

    identified_entities[current_question_word] = identified_entities_list_extension
...
def syntax_text(answer_from_user):    

    if isinstance(answer_from_user, six.binary_type):
        answer_from_user = answer_from_user.decode('utf-8')
...
def fetch_entities_from_google_api(answer_from_user):
    ...
    response = client.analyze_entities(document).entities    
    return response
